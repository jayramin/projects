...
$RetailersBalanceList = $fn->GetGeneralUserBalanceData();
//echo "<pre>";
//print_r($RetailersBalanceList[0]['RetailerID']);
//exit;
?>
<!--Content-->
<div class="content-wrapper">
   <div class="row">
    <div class="page-title">
        ...
        <div class="col-lg-8">
                <div>
                <h1><i class="fa fa-dashboard"></i> Payment Management</h1>
            </div>
            <div>
...
//                        print_r($RetailersBalanceList);
                    if($_REQUEST['method']=='edit'){
                        echo 'Edit General User Balance';
                    }else if($_REQUEST['method']=='add'){
                        echo 'New General User Balance Entry';
                    }else{
                        echo 'View General User Balance List';
                    } ?>
                        </li>
                </ul>
            </div>
...
//exit;
                    ...
                    if ($_REQUEST['method'] == '') { ?>
                <div class="col-lg-12">
                    <a href='view_general_user_make_payment&method=add' class="btn btn-sm btn-success btn-labeled text-uppercase waves pull-right waves-effect waves-float" title="Click here to add new record"><span class="btn-label"><i class="fa fa-plus-square"></i></span>  Generate Payment Bill</a>
                </div>
<!--                <div class="col-lg-6">
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            if (isset($_REQUEST['method']) && ($_REQUEST['method'] == 'add' || $_REQUEST['method'] == 'edit')) {
                  require_once 'forms/form_create_general_user_balance.php';
...
<!--                                                        <div class="btn-group">
                                                            <button class="btn waves waves-effect waves-float btn-sm btn-<?php echo $Color; ?>" title="Click here to change status of this entry" onclick="ChangeStatus('<?php echo 'b_general_user_balance'; ?>', '<?php echo $Value['GeneralUserBalanceID']; ?>', '<?php echo $NewStatus; ?>','GeneralUserBalanceID', 'is_active','view_retailers_balance_details');"><i class="fa <?php echo $ICON; ?>"></i></button> 
                                                        </div>-->
                                                        <div class="btn-group">
                                                            <a class="btn waves waves-effect waves-float btn-sm btn-info" title="Click here to edit this entry" href="<?php echo  'view_general_user_make_payment&method=edit&GeneralUserBalanceID=' . $Value['GeneralUserBalanceID'] ;?>"><i class="fa fa-edit"></i></a>
                                                        </div>
                                                        <div class="btn-group">
                                                            <button class="btn waves waves-effect waves-float btn-sm btn-danger" title="Click here to remove this entry" onclick="DeleteSingleRecord('<?php echo 'b_general_user_balance'; ?>', '<?php echo $Value['GeneralUserBalanceID']; ?>', 'D','GeneralUserBalanceID', 'is_active','view_retailers_balance_details','b_general_user_balance','GeneralUserBalanceID,GeneralUserBalanceID');"><i class="fa fa-trash-o"></i></button>
