...
//exit;
        ...
        if (move_uploaded_file($sourcePath, "uploads/ProfilePicture/" . $targetPath)) {
            compress("uploads/ProfilePicture/" . $targetPath,"uploads/ProfilePicture/" . $targetPath, $ImageQuality);
            echo json_encode(array('ResponseCode' => 1, 'Message' => 'Profile Picture Uploaded Successfully', 'FileName' => $targetPath, 'FilePath' => ADMIN_URL.'/uploads/ProfilePicture/'.$targetPath));
        }else {
...
//            echo 'File Uploaded Successfully';
    ...
    }else if (is_uploaded_file($_FILES['BookImage']['tmp_name'])) {
        
        $sourcePath = $_FILES['BookImage']['tmp_name'];
        $targetPath = time() . "-" . $_FILES['BookImage']['name'];
        if (move_uploaded_file($sourcePath, "uploads/BookImage/" . $targetPath)) {
            compress("uploads/BookImage/" . $targetPath,"uploads/BookImage/" . $targetPath, $ImageQuality);
//            echo $targetPath;
//            echo '1';
//            echo 'File Uploaded Successfully';
        echo json_encode(array('ResponseCode' => 1, 'Message' => 'Book Image Uploaded Successfully', 'FileName' => $targetPath, 'FilePath' => ADMIN_URL.'/uploads/BookImage/'.$targetPath));
        }else {
            echo json_encode(array('ResponseCode' => 0, 'Message' => 'Failed ! Please try again'));
        }
    }
