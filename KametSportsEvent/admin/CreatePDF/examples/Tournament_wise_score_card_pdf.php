...
$pdf->SetSubject("Tournament Score Card");
...
$html = '<h1>Score Card For Tournament:-> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="color:green;">'.ucwords($Tournaments['TournamentName']).'</span></h1><table style="width:100%;" cellspacing="0" cellpadding="5" border="1">
                                <thead>
                                    <tr>
                                        <th>'.SrNo.'</th>
                                        <th>'.TeamName.'</th>
                                        <th>'.TotalMatchPlayed.'</th>
                                        <th>'.TotalMatchWon.'</th>
                                        <th>'.TotalMatchLoose.'</th>
                                        <th>'.TotalMatchAbandoned.'</th>
                                    </tr>
                                </thead>
                                <tbody>';
foreach ($PData AS $Key => $Value) {
    $html.="<tr>
                                                <td>".($Key + 1)."</td>
                                                <td>".ucwords($Value['TeamName'])."</td>
                                                <td>".$Value['TotalMatch']."</td>
                                                <td>".$Value['MatchWinCount']."</td>
                                                <td>".$Value['MatchLooseCount']."</td>
                                                <td>".$Value['TotalAbandond']."</td>
                                            </tr>";
}
$html.= '</tbody></table><br>';
...
$FileName = 'Score Card For - '.ucwords($Tournaments['TournamentName']).' ' . date('Y-m-d H:i:s') . '.pdf';
ob_end_clean();
