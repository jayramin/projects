...
class DataTransaction {
    ...
    public function update($TableName, $DataObject, $Condition) {
        ...
        //print_r($sql_update);
        try {
            //echo "'UPDATE `' . $TableName . '` SET ' . $sql_update . ' WHERE ' . implode(' AND ', $GetData)";
...
//            echo $UPdateParams;
    ...
    public function insert($Data, $ViewName) {
        ...
        //echo 'INSERT INTO `' . $ViewName . '` (' . $keys . ') VALUES (' . $valsDebug . ')';
//        exit;
        try {
...
//            echo $stmt;
    ...
    public function selectdata($ViewName, $Columns, $Condition) {
      ...
      $query = 'SELECT '.$Cols. ' FROM `' . $ViewName.'`  WHERE ' . $Cond;
        try {
